...
def xml_to_property(property_node):
    ...
    # keys
    key_node, = (node for node in property_node.childNodes if
                 node.nodeName == "keys")
    key_type = key_node.getAttribute("type")
    key_descr = key_node.getAttribute("description")

    # values
    value_node, = (node for node in property_node.childNodes if
                   node.nodeName == "values")
    value_type = value_node.getAttribute("type")
    value_descr = value_node.getAttribute("description")

    # datas
    datas_node, = (node for node in property_node.childNodes if
                   node.nodeName == "datas")
