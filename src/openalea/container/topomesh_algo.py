...
def merge_wisps(mesh, scale, wid1, wid2):
    ...
    if scale < mesh.degree():
        for rid in set(mesh.regions(scale, wid1)) | set(
                mesh.regions(scale, wid2)):
            mesh.link(scale + 1, rid, nwid)
    # connect with borders
    for bid in (set(mesh.borders(scale, wid1)) | set(
            mesh.borders(scale, wid2))) - frontier:
...
def topo_triangulate_polygon(pids):
    ...
    if len(pids) < 3:
        raise UserWarning(
            "unable to triangulate polygon with less than 3 points")
    elif len(pids) == 3:
        return [pids]
    else:
        polygons = [pids]
...
def clean_duplicated_borders(mesh, outer=True):
    ...
    for deg in xrange(mesh.degree() - 1, mesh.degree() - 2, -1):
        ...
        if outer:
            ...
            duplicated = [(k, v) for k, v in bd.iteritems() if
                          len(k) > 1 and len(v) > 1]
            ...
            for j in xrange(len(paths)):  # walk trough each path
                ...
                for nid in neighbors:
                    if nid == path[0]:  # closed path
                        ...
                        pth.sort()
                        cycles[tuple(
                            pth)] = path  # necessary to not duplicate paths
                    else:
                        extrs = set(mesh.borders(scale, nid))
                        extrs.remove(extr)
                        for bid in extrs:
                            paths.append((path + [nid],
                                          bid))  # increase the size of the path
