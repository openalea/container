...
class PropertyTopomesh(Topomesh):
        ...
        if isinstance(key, tuple) and len(key) == 2:
            ...
            if wid in self.wisps(degree):
                ...
                for property_name in self.wisp_property_names(degree=degree):
                    if self._wisp_properties[degree][property_name].has_key(
                            wid):
                        property_wid[wid][property_name] = \
                        self._wisp_properties[degree][property_name][wid]
                return property_wid
            else:
            ...
            if (degree <= self._degree):
                ...
                for wid in self.wisps(degree):
                    property_wids[wid] = {}
                    for property_name in self.wisp_property_names(
                            degree=degree):
                        if self._wisp_properties[degree][property_name].has_key(
                                wid):
                            property_wids[wid][property_name] = \
                            self._wisp_properties[degree][property_name][wid]
    ...
    def wisp_property(self, property_name, degree, wids=None):
        ...
        try:
            ...
            raise PropertyError(
                "property " + property_name + " is undefined on wisps of degree " + str(
                    degree))
    ...
    def add_wisp_property(self, property_name, degree, values=None):
        ...
        if property_name in self._wisp_properties[degree]:
            raise PropertyError(
                "property " + property_name + " is already defined on wisps of degree " + str(
                    degree))
        if values is None:
            values = array_dict()
    ...
    def has_wisp_property(self, property_name, degree, is_computed=False):
        if property_name in self._wisp_properties[degree]:
            return (not is_computed) or (
            len(self._wisp_properties[degree][property_name]) == self.nb_wisps(
                degree))
        else:
            return False
    ...
    def interface_property(self, property_name, degree, wids=None):
        ...
        try:
            ...
            raise PropertyError(
                "property " + property_name + " is undefined on interfaces of degree " + str(
                    degree))
    ...
    def add_interface_property(self, property_name, degree, values=None):
        ...
        if property_name in self._interface_properties[degree]:
            raise PropertyError(
                "property " + property_name + " is already defined on interfaces of degree " + str(
                    degree))
        if values is None:
            values = array_dict()
        elif isinstance(values, np.ndarray):
    ...
    def update_interface_property(self, property_name, degree, values,
                                  keys=None):
        """todo"""
        if not (isinstance(values, np.ndarray) or isinstance(values,
                                                             dict) or isinstance(
        ...
        if isinstance(values, np.ndarray):
            if keys is None:
                keys = np.array(self._interface.keys())
            self._interface_properties[degree][property_name].update(values,
                                                                     keys=keys,
            ...
            self._interface_properties[degree][property_name].update(
                np.array(values.values()), keys=keys, ignore_missing_keys=False)
        elif isinstance(values, array_dict):
            if keys is None:
                keys = values.keys()
            self._interface_properties[degree][property_name].update(
    ...
    def has_interface_property(self, property_name, degree, is_computed=False):
        if property_name in self._interface_properties[degree]:
            return (not is_computed) or (
            len(self._interface_properties[degree][property_name]) == len(
                list(self.interfaces(degree))))
        else:
            return False
    ...
    def interface(self, degree, wid1, wid2):
        ...
        if not wid2 in self.border_neighbors(degree, wid1):
            ...
            return np.intersect1d(
                np.where(np.array(self._interface[degree].values()) == wid1)[0],
                np.where(np.array(self._interface[degree].values()) == wid2)[
                    0])[0]
