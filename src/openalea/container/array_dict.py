...
class array_dict:
    ...
    def __setitem__(self, key, value):
        ...
        if key in self._keys:
            ...
            if len(self) == 0:
                ...
                self._values = np.append(self._values, np.array([value]),
                                         axis=0)
    ...
    def __str__(self):
        ...
        for i, k in enumerate(self._keys):
            self.dict_string = self.dict_string + str(k) + ": " + str(
                self._values[self._index[k]])
    ...
    def __repr__(self):
        ...
        if len(self) < 10:
            ...
            for i in xrange(3):
                self.dict_string = self.dict_string + str(
                    self._keys[i]) + ": " + str(self._values[i]) + ","
            self.dict_string = self.dict_string + "..."
            for i in xrange(3):
                self.dict_string = self.dict_string + "," + str(
                    self._keys[len(self) - 3 + i]) + ": " + str(
                    self._values[len(self) - 3 + i])
        self.dict_string = self.dict_string + "}\n"
        return self.dict_string

    def to_dict(self):
        return dict([(tuple(k), self._values[i]) if isiterable(k) else (
        k, self._values[i]) for i, k in enumerate(self._keys)])
    ...
    def update(self, values, keys=None, ignore_missing_keys=True,
               erase_missing_keys=True):
        if keys is None or (
                len(keys) == len(self._keys) and (keys == self.keys()).all()):
            ...
            if len(keys_to_assign) != len(keys):
                key_range = np.array(
                    np.sum(np.arange(keys.size), keys, index=keys_to_assign),
                    int)
                ...
                if ignore_missing_keys:
                    ...
                    missed_keys = np.array(
                        tuple(set(keys).difference(set(self._keys))))
                    newindex = np.arange(len(self._values),
                                         len(self._values) + len(missed_keys))
                    ...
                    if missed_keys.max() < len(self._index):
                        ...
                        self._index = new_index
                    key_range = np.array(
                        np.sum(np.arange(keys.size), keys, index=missed_keys),
                        int)
                    if len(self) == 0:
                        self._values = np.array(values[key_range])
                    else:
                        self._values = np.append(self._values,
                                                 values[key_range], axis=0)
            else:
                if len(keys_to_assign) != len(self._keys) or (
                    keys != self.keys()).any():
